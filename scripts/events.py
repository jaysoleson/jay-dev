...
class Events:
        ...
        if Cat.grief_strings:
            ...
            for ID in Cat.grief_strings.copy():
                ...
                if isinstance(check_cat, Cat):
                    if check_cat.dead or check_cat.outside or "undead" in check_cat.illnesses:
    ...
    def new_infection(self, custom_type=None):
        """
        Reinfecting the Clan
        """
        ...
        if custom_type:
            ...
            new_inftype = random.choice(inftypes)
        
        if current_type == custom_type:
            print("New type and old type are the same-- not rerolling everything else.")
            return
        ...
        update_infection_info("cure", [herb1, herb2, herb3, herb4])
        update_infection_info("logs", [])
        update_infection_info("spread_by", random.choice(["air", "bite"]))
        update_infection_info("cure_discovered", False)
    ...
    def yourcat_infection(self, you):
        ...
        if num == 1:
            you.get_ill("stage one infection")
    ...
    def generate_mate_events(self):
        if len(game.clan.your_cat.mates) > 0:
            if random.randint(1,20) == 1:
                ...
                if mate1.dead or mate1.outside or "undead" in mate1.illnesses:
                    return
    ...
    def generate_df_events(self):
        ...
        if random.randint(1,30) == 1:
            ...
            counter = 0
            while r_clanmate.dead or r_clanmate.outside or r_clanmate.status in ['kitten', 'newborn', 'deputy', 'leader'] or r_clanmate.joined_df or r_clanmate.ID == game.clan.your_cat.ID or "undead" in r_clanmate.illnesses:
                counter+=1
                if counter > 15:
                    return
                r_clanmate = Cat.all_cats.get(random.choice(game.clan.clan_cats))
    ...
    def handle_lead_den_event(self):
        ...
        if game.clan.clan_settings["lead_den_clan_event"]:
            ...
            if gathering_cat.exiled or gathering_cat.dead or gathering_cat.outside or "undead" in gathering_cat.illnesses:
                return
                            ...
                            if invited_cat.infected_for > 0:
                                ...
                                if game.clan.infection["clan_infected"] is False:
                                    ...
                                    if "cure_found" in get_infection_info("logs"):
                                        get_infection_info("logs").append("start")
                                        custom_type = None
                                        for illness in invited_cat.illnesses:
                                            if "infection" in illness:
                                                custom_type = invited_cat.illnesses[illness]["type"]
                                        self.new_infection(custom_type=custom_type)
            ...
            for cat in relevant_cats:
                ...
                if game.clan.clan_settings.get(
                    ...
                    if not int(random.random() * chance):  # 1/chance
                        ...
                        if not (chosen_illness == "stage one infection" and cat.ID == game.clan.your_cat.ID):
    ...
    def perform_ceremonies(self, cat):
        ...
        if game.clan.deputy:
            if game.clan.deputy is not None and \
                ...
                game.cur_events_list.append(
                    Single_Event(text, types, game.clan.deputy.ID))
                ...
                if cat.status == 'kitten':
                    ...
                    med_cat_list = [i for i in Cat.all_cats_list if
                                    i.status in ["medicine cat", "medicine cat apprentice"] and not (
                                            i.dead or i.outside) and "undead" not in i.illnesses]
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared", LG_TYPE=""):
        ...
        try:
            ...
            for c in cat.former_mentor:
                if not (
                    (Cat.fetch_cat(c).dead or Cat.fetch_cat(c).outside or Cat.fetch_cat(c).shunned > 0) and
                    "undead" not in Cat.fetch_cat(c).illnesses
                    ):
    ...
    def gain_accessories(self, cat):
        ...
        if cat.dead or cat.outside or "undead" in cat.illnesses:
            return
    ...
    def handle_outbreaks(self, cat):
        ...
        for illness in cat.illnesses:
            ...
            if illness in ["stage one infection", "stage two infection", "stage three infection", "stage four infection"] and get_infection_info("spread_by") == "bite":
                continue
            ...
            if not int(random.random() * chance):  # 1/chance to infect
                ...
                if illness in ["stage one infection", "stage two infection", "stage three infection", "stage four infection"]:
                    # adjust alive cats list for infection
                    alive_cats = list(
                        filter(
                            lambda kitty:
                            (not kitty.dead and kitty.ID != game.clan.your_cat.ID and
                             not kitty.outside and kitty.infected_for == 0), Cat.all_cats.values()))
                    alive_count = len(alive_cats)
                ...
                for sick_meowmeow in infected_cats:
                    ...
                    if illness in ["stage two infection", "stage three infection", "stage four infection"]:
                        sick_meowmeow.get_ill(
                            "stage one infection", event_triggered=True)
                    else:
                        sick_meowmeow.get_ill(
                ...
                elif illness in ["stage one infection", "stage two infection", "stage three infection", "stage four infection"]:
    ...
    def check_and_promote_leader(self):
        ...
        # check for leader
        if game.clan.leader:
            leader_invalid = game.clan.leader.dead or game.clan.leader.outside or game.clan.leader.infected_for > 0
        else:
            leader_invalid = True

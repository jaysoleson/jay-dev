...
from scripts.utility import get_text_box_theme, ui_scale, ui_scale_blit, ui_scale_offset, generate_map
...
class MakeClanScreen(Screens):
    ...
    def screen_switches(self):
        ...
        self.baron_img = pygame.transform.scale(
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            elif self.sub_screen == "choose map":
                self.handle_choose_map_event(event)
    ...
    def handle_name_clan_event(self, event):
        if event.ui_element == self.elements["random"]:
            ...
            self.colour = choice(self.barony_colours)
            self.refresh_text_and_buttons()
        elif event.ui_element == self.elements["next_step"]:
            self.open_choose_baron()
        ...
        for button in self.elements:
            if button.startswith("colour"):
                colour = button.split("_")[1]
                if event.ui_element == self.elements[button]:
                    self.colour = colour
                    self.refresh_text_and_buttons()
    ...
    def handle_choose_baron_event(self, event):
    ...
    def handle_choose_regent_event(self, event):
        if event.ui_element == self.elements["previous_step"]:
            ...
            if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                clicked_cat = event.ui_element.return_cat_object()
                if clicked_cat.age not in ("newborn", "kitten", "adolescent"):
                    self.regent = clicked_cat
                    self.selected_cat = None
                    self.open_choose_doctor()
            elif event.ui_element.return_cat_object() != self.baron:
                self.selected_cat = event.ui_element.return_cat_object()
                self.refresh_cat_images_and_info(self.selected_cat)
                self.refresh_text_and_buttons()
        elif event.ui_element == self.elements["select_cat"]:
            self.regent = self.selected_cat
            self.selected_cat = None
            self.open_choose_doctor()
    ...
    def handle_choose_map_event(self, event):
        if event.ui_element == self.elements["continue"]:
            self.open_clan_saved_screen()
        elif event.ui_element in [
            self.elements["cycle_border_left"],
            self.elements["cycle_border_right"]
            ]:
            clan_num = (game.switches["creation_other_clans"][0])
            possible_presets = game.badlands_info["tilemaps"][str(clan_num)]
            if game.switches["creation_other_clans"][1] in possible_presets:
                current_index = possible_presets.index(game.switches["creation_other_clans"][1])

                if event.ui_element == self.elements["cycle_border_left"]:
                    if current_index == 0:
                        new_index = len(possible_presets) - 1
                    else:
                        new_index = current_index - 1
                else:
                    if current_index == len(possible_presets) - 1:
                        new_index = 0
                    else:
                        new_index = current_index + 1
                try:
                    game.switches["creation_other_clans"][1] = []
                    chosen_map = possible_presets[new_index]
                    for tile_list in chosen_map:
                        game.switches["creation_other_clans"][1].append(tile_list)
                except Exception as e:
                    print("Map Error:", e)
                    print(current_index, new_index, len(possible_presets))

    def handle_saved_clan_event(self, event):
        # BL
        if event.ui_element == self.elements["random"]:
            self.elements["baron_name_entry"].set_text(self.random_clan_name())
        elif event.ui_element == self.elements["continue"]:
            new_name = sub(
                r"[^A-Za-z0-9 ]+", "", self.elements["baron_name_entry"].get_text()
            ).strip()
            if not new_name:
                self.elements["error"].set_text("Your Baron's name cannot be empty")
                self.elements["error"].show()
                return
            self.baron.name.prefix = new_name
    ...
    def on_use(self):
        ...
        # BL
        # baron name text box
        if self.sub_screen == "saved screen":
            if self.elements["baron_name_entry"].get_text() == "":
                self.elements["continue"].disable()
            elif self.elements["baron_name_entry"].get_text().startswith(" "):
                self.elements["error"].set_text(
                    "screens.make_clan.error_clan_name_space"
                )
                self.elements["error"].show()
        ...
        if self.sub_screen == "choose map":
            self.create_map()
    
    def random_borders(self):
        game.switches["creation_other_clans"][1] = []
        clan_num = (game.switches["creation_other_clans"][0])

        possible_presets = game.badlands_info["tilemaps"][str(clan_num)]

        chosen_map = choice(possible_presets)
        for tile_list in chosen_map:
            game.switches["creation_other_clans"][1].append(tile_list)
        ...
        # abandoned.... for neow.....
        # all_tiles = [
        #     "1-1", "1-2", "2-1", "2-2", "3-1", "3-2", "3-3", "4-3", "1-3", "1-4", "1-5", "2-3", "2-4", "2-5", "3-4", "3-5",
        #     "1-6", "1-7", "2-6", "2-7", "3-6", "3-7", "4-5", "4-6", "4-7", "5-5", "5-6", "5-7", "6-6", "6-7", "7-6", "7-7",
        #     "5-4", "6-3", "6-4", "6-5", "7-2", "7-3", "7-4", "7-5", "4-1", "4-2", "5-1", "5-2", "5-3", "6-1", "6-2", "7-1"
        # ]
        ...
        #     if current_barony >= clan_num:
        #         current_barony = 0
        #     if str(current_barony) not in tile_numbers.keys():
        #         tile_numbers.update({str(current_barony): 0})
        #     tile_numbers[str(current_barony)] += 1
        #     current_barony += 1
        ...
        #     first_tile = choice(all_tiles)
        #     clan_tiles.update({i: [first_tile]})
        #     print("Updating Clan Tiles:", i, ":", first_tile)
        #     all_tiles.remove(first_tile)
        
        # print(clan_tiles)

        # # for i in range(len(all_tiles)):
        # for barony in range(clan_num):
        #     for i in range(round(len(all_tiles)/clan_num)):
        #         print("Getting new tile from", clan_tiles[barony][-1])

        #         tile_index = 0
                
        #         while tile_index < len(clan_tiles[barony]):
        #             x, y = clan_tiles[barony][tile_index].split("-")
        #             x = int(x)
        #             y = int(y)
        ...
        #                 if direction in all_tiles:
        #                     possible_tiles.append(direction)

        #             if possible_tiles:
        #                 break  # Exit loop if we found possible tiles
        #             else:
        #                 tile_index += 1  # Try next tile
                
        #         if not possible_tiles or tile_index > len(clan_tiles[barony]):
        #             print(barony, "No possible tiles.")
        #             continue
                
        #         new_tile = choice(possible_tiles)
        #         clan_tiles[barony].append(new_tile)
        #         all_tiles.remove(new_tile)

        # for index, tile_list in clan_tiles.items():
        #     game.switches["creation_other_clans"][1].append(tile_list)

        # print("FINISHED")
        # print(game.switches["creation_other_clans"][1])
    ...
    def refresh_text_and_buttons(self):
        ...
        # BL
            ...
            for button in self.elements:
                if button.startswith("colour"):
                    self.elements[button].enable()
            self.elements["colour_" + self.colour].disable()

        # Show the error message if you try to choose a child for baron, regent, or med cat.
        elif self.sub_screen in ("choose baron", "choose regent", "choose med cat"):
    ...
    def open_name_clan(self):
        ...
        self.elements["random"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((184, 595), (34, 34))),
            Icon.DICE,
        ...
        self.elements["name_entry"] = pygame_gui.elements.UITextEntryLine(
            ui_scale(pygame.Rect((225, 597), (140, 29))),
            manager=MANAGER,
        )
        ...
        self.elements["clan"] = pygame_gui.elements.UITextBox(
            "Territory",
            ui_scale(pygame.Rect((365, 590), (100, 35))),
            object_id="#text_box_30_horizcenter_light",
            manager=MANAGER,
        )
        self.elements["reset_name"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((475, 595), (134, 30))),
        ...
        self.elements["title"] = pygame_gui.elements.UITextBox(
            ...
            ui_scale(pygame.Rect((0, 535), (500, 40))),
            object_id="@clangen_32",
            anchors={"centerx": "centerx"},
        )

        # BL
        self.elements["colour_title"] = pygame_gui.elements.UITextBox(
            "screens.make_clan.choose_colour_title",
            ui_scale(pygame.Rect((0, 122), (400, 40))),
            object_id="@clangen_32",
            anchors={"centerx": "centerx"},
        )
        self.elements["colour_subtitle"] = pygame_gui.elements.UITextBox(
            "screens.make_clan.choose_colour_subtitle",
            ui_scale(pygame.Rect((0, 160), (600, 100))),
            object_id=get_text_box_theme("#text_box_26_horizcenter"),
            anchors={"centerx": "centerx"},
        )

        self.colour_buttons_container = UIContainer(
            ui_scale(pygame.Rect((0, 180), (500, 150))),
        ...
        x_values = [20, 140, 260, 380]
        y_values = [20, 60, 100]

        x = 0
        y = 0
        for colour in self.barony_colours:
            self.elements[f"colour_{colour}"] = UISurfaceImageButton(
                ui_scale(pygame.Rect((x_values[x], y_values[y]), (100, 30))),
                colour.capitalize(),
                get_button_dict(ButtonStyles.SQUOVAL, (100, 30)),
                object_id="@buttonstyles_squoval",
                container=self.colour_buttons_container,
                manager=MANAGER,
            )
            if x == (len(x_values) - 1):
                y += 1
                ...
                continue
            x += 1
        self.elements[f"colour_{self.colour}"].disable()
        # ---

    def clan_name_header(self):
        self.elements["name_backdrop"] = pygame_gui.elements.UIImage(
            ui_scale(pygame.Rect((0, 100), (256, 50))),
            self.clan_frame_img,
            manager=MANAGER,
            anchors={"centerx": "centerx"}
        )
        self.elements["clan_name"] = pygame_gui.elements.UITextBox(
            "Assemble Your Barony",
            ui_scale(pygame.Rect((0, 97), (256, 50))),
            object_id=ObjectID("#text_box_30_horizcenter_vertcenter", "#dark"),
            manager=MANAGER,
            anchors={"centerx": "centerx"}
        )

    def open_choose_baron(self):
        """Set up the screen for the choose baron phase."""
        self.clear_all_page()
        self.sub_screen = "choose baron"
        ...
        self.elements["select_cat"] = UIImageButton(
            ...
            "screens.make_clan.choose_baron",
    ...
    def open_choose_regent(self):
        """Open sub-page to select regent."""
        self.clear_all_page()
        self.sub_screen = "choose regent"
        ...
        self.clan_name_header()
        self.elements["title"] = pygame_gui.elements.UITextBox(
            "screens.make_clan.regent_title",
            ui_scale(pygame.Rect((0, 610), (800, 90))),
            object_id="@clangen_32",
            anchors={"centerx": "centerx"},
        )
        ...
        self.elements["select_cat"] = UIImageButton(
            ui_scale(pygame.Rect((209, 348), (384, 52))),
            "screens.make_clan.choose_regent",
            object_id="#support_leader_button",
            starting_height=2,
            visible=False,
            manager=MANAGER,
        ...
        # Error message, to appear if you can't choose that cat.
        self.elements["error_message"] = pygame_gui.elements.UITextBox(
            "screens.make_clan.error_too_young_regent",
            ui_scale(pygame.Rect((150, 353), (500, 55))),
            object_id=get_text_box_theme("#text_box_30_horizcenter_red"),
            visible=False,
            manager=MANAGER,
    ...
    def open_choose_symbol(self):
        ...
        self.text["baron"] = pygame_gui.elements.UILabel(
            ui_scale(pygame.Rect((0, 5), (-1, -1))),
            text="screens.make_clan.symbol_baron",
            container=self.elements["text_container"],
            object_id=get_text_box_theme("#text_box_30_horizleft"),
            manager=MANAGER,
            text_kwargs={"prefix": self.baron.name.prefix},
            anchors={
    ...
    def open_choose_map(self):
        self.clear_all_page()
        self.sub_screen = "choose map"
        self.elements["continue"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((0, 550), (102, 30))),
            "buttons.continue",
        ...
        game.switches["creation_other_clans"][0] = randint(3,5)

        self.elements["number_other_clans"] = pygame_gui.elements.UITextBox(
            html_text="Other Clans: " + str(game.switches["creation_other_clans"][0]),
            relative_rect=ui_scale(pygame.Rect((0, 40), (350, -1))),
            manager=MANAGER,
            object_id=get_text_box_theme("#text_box_30_horizcenter"),
            anchors={"centerx": "centerx"}
        )
        ...
        territory_string = ""
        for index, item in enumerate(game.switches["creation_other_clans"]):
            if index == 0:
                continue
            territory_string = territory_string + f"<b>Clan {index}: {item}"

        self.elements["choose_text"] = pygame_gui.elements.UITextBox(
            html_text="Choose a map",
            relative_rect=ui_scale(pygame.Rect((0, 470), (170, -1))),
            manager=MANAGER,
            object_id=get_text_box_theme("#text_box_34_horizcenter"),
            anchors={"centerx": "centerx"}
        )
        self.elements["cycle_border_right"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((540, 550), (32, 32))),
            Icon.ARROW_RIGHT,
            get_button_dict(ButtonStyles.ICON, (32, 32)),
            object_id="@buttonstyles_icon"
        )
        self.elements["cycle_border_left"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((260, 550), (32, 32))),
            Icon.ARROW_LEFT,
            get_button_dict(ButtonStyles.ICON, (32, 32)),
            object_id="@buttonstyles_icon"
        )
    ...
    def open_clan_saved_screen(self):
        ...
        # BL
        self.elements["random"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((275, 302), (34, 34))),
            Icon.DICE,
            get_button_dict(ButtonStyles.ICON, (34, 34)),
            object_id="@buttonstyles_icon",
            manager=MANAGER,
        ...
        self.elements["error"] = pygame_gui.elements.UITextBox(
            "",
            ui_scale(pygame.Rect((506, 200), (596, -1))),
            manager=MANAGER,
            object_id="#default_dark",
            visible=False,
        )
        self.elements["name_baron"] = pygame_gui.elements.UITextBox(
            "screens.make_clan.name_baron",
            ui_scale(pygame.Rect((0, 250), (600, 30))),
            object_id=get_text_box_theme("#text_box_30_horizcenter"),
            manager=MANAGER,
            anchors={"centerx": "centerx"}
        )

        self.elements["baron_name_entry"] = pygame_gui.elements.UITextEntryLine(
            ui_scale(pygame.Rect((0, 300), (140, 40))),
            manager=MANAGER,
            initial_text=str(self.baron.name),
            anchors={"centerx": "centerx"}
        )
        self.elements["baron_name_entry"].set_allowed_characters(
            list("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_- ")
        )
        self.elements["baron_name_entry"].set_text_length_limit(11)
        # ---
        ...
        self.elements["baron_image"] = pygame_gui.elements.UIImage(
            ui_scale(pygame.Rect((350, 125), (100, 100))),
            pygame.transform.scale(
                self.baron.sprite, ui_scale_dimensions((100, 100))
            ),
            starting_height=1,
            manager=MANAGER,
        )
        self.elements["continue"] = UISurfaceImageButton(
            ui_scale(pygame.Rect((346, 450), (102, 30))),
            "buttons.continue",
            get_button_dict(ButtonStyles.SQUOVAL, (102, 30)),
            object_id="@buttonstyles_squoval",
            sound_id="save",
    ...
    def save_clan(self):
        ...
        game.clan = Clan(
            ...
            colour=self.colour,
            territory=game.switches["creation_other_clans"][1][0],
            export=self.export
        )
        # BL
    ...
    def create_map(self):
        ...
        for y in range(1, 8):
            for x in range(1, 8):
                for clan_territory in game.switches["creation_other_clans"][1]:
                    generate_map(Cat, screen, x, y, x_pos, y_pos, clan_territory)
                x_pos += 50
            y_pos += 50
            if game.settings["fullscreen"]:
                x_pos = 295

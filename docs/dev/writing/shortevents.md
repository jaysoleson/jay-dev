...
| "status:{some_status}"                      | Cats will join with this status. Include "doctor", "colt", "mediator", "kitten", "newborn", "apprentice doctor", etc, but not leader or deputy. Default for not-litters is warrior. Be very careful specifying both age and status-  there is no extra check to ensure they make sense together.                                                   |
